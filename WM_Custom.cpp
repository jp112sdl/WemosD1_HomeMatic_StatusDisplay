...
WiFiManagerParameter::WiFiManagerParameter(const char *id, const char *placeholder, const char *defaultValue, int length) {
  init(id, placeholder, defaultValue, length, INPUT_TEXT, "");
}
...
void WiFiManager::handleRoot() {
  ...
  page += _apName;
  page += "</h1>";
  page += F("<center><h3>Wemos - HomeMatic - Statusdisplay</h3></center>");
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    if (_params[i]->getType() == INPUT_TEXT) {
    ...
    if (_params[i]->getType() == INPUT_CHECKBOX) {
    ...
    if (_params[i]->getType() == INPUT_COMBOBOX) {
    ...
    if (_params[i]->getType() == INPUT_COLOR) {
      String pitem = FPSTR(HTTP_FORM_PARAM_COLOR);
      pitem.replace("{i}", _params[i]->getID());
      pitem.replace("{n}", _params[i]->getID());
      pitem.replace("{p}", _params[i]->getPlaceholder());
      snprintf(parLength, 2, "%d", _params[i]->getValueLength());
    ...
    if (_params[i]->getType() == INPUT_PASSWORD) {
