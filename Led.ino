void initLEDs() {
  ...
  switch (GlobalConfig.SelectedEOrder) {
    case _RGB:
      DEBUG(F("Farbreihenfolge RGB"), "initLEDs()", _slInformational);
      FastLED.addLeds<LED_TYPE, LED_PIN, RGB>(LEDConfig.leds, GlobalConfig.NumLeds);
      break;
    case _GRB:
      DEBUG(F("Farbreihenfolge GRB"), "initLEDs()", _slInformational);
...
void setLed(int led, int hex) {
  if (DisplayState == Wake) {
    DEBUG("setLed for LED " + String(led) + " value = " + String(hex, HEX) + ", DisplayState == WAKE", "setLed()", _slInformational);
    if (led == -1) {
    ...
    DEBUG("setLed for LED " + String(led) + " value = " + String(hex, HEX) + ", DisplayState == SLEEP", "setLed()", _slInformational);
    if (led == -1)  {
      for (int i = 0; i < GlobalConfig.NumLeds; i++) {
